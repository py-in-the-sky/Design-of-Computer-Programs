"""
in hand: blank = '_'
on board it will take value (letter) given
what in between?  use lowercase letter?

need to choose letter value for a blank to create a valid word
    - remove blank from letters when chosen
    - assign lowercase letter value to produce valid word
need to appropriately score this letter as worth 0
    - rewrote POINTS to take care of this
"""
...
POINTS = dict(A=1, B=3, C=3, D=2, E=1, F=4, G=2, H=4, I=1, J=8, K=5, L=1, M=3, N=1, O=1,
              P=3, Q=10, R=1, S=1, T=1, U=1, V=4, W=4, X=8, Y=4, Z=10, _=0)
POINTS = {L: POINTS.get(L,0) for L in string.letters+'_'}  # CHANGE MADE HERE
    ...
    if pre in PREFIXES:       
        ...
        if is_letter(sq):
            ...
            for L in hand:
                ...
                if L is '_':  # CHANGES MADE HERE
                    for lwr in string.lowercase:
                        if lwr.upper() in possibilities:
                            add_suffixes(hand.replace(L, '', 1), pre+lwr, start, row, results)
    return results
...
def find_prefixes(hand, pre='', results=None):
    ...
    if pre in PREFIXES:
        for L in hand:
            if L is '_':  # CHANGES MADE HERE
                for lwr in string.lowercase:
                    find_prefixes(hand.replace(L, '', 1), pre+lwr, results)
            else:
